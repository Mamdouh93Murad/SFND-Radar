...
bandwidth = speed_light / ( 2 * range_res );
Tchirp = (5.5*2*max_range)/speed_light;
...
for i=1:length(t)         
    ...
    range_ = range + t(i)*velocity;
    % *%TODO* :
    %For each time sample we need update the transmitted and
    %received signal. 
    time_delay = (range_ * 2) / speed_light;
    Tx(i) = cos(2 * pi * (fc * t(i) + (slope * t(i) ^ 2) / 2));
    Rx(i)  = cos ( 2 * pi * (fc * (t(i) - time_delay) + (slope * (t(i) - time_delay)^2) / 2));
...
%normalize.
signal_fft = fft(reMix, Nr);
norm_signal_fft = signal_fft / length(signal_fft);
 % *%TODO* :
% Take the absolute value of FFT output
abs_signal_fft = abs(norm_signal_fft);
 % *%TODO* :
% Output of FFT is double sided signal, but we are interested in only one side of the spectrum.
% Hence we throw out half of the samples.
up_signal_fft = abs_signal_fft(1:length(signal_fft)/2+1);
%plotting the range
...
subplot(2,1,1)
 ...
 % *%TODO* :
 % plot FFT output 
f = Nr*(0:length(signal_fft)/2)/length(signal_fft);
plot(f, up_signal_fft);
axis ([0 200 0 1]);
...
figure('Name', 'CFAR');
surf(doppler_axis,range_axis,RDM_Filtered);
colorbar;
